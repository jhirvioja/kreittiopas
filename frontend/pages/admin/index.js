...
function Toggle(props) {
  ...
  const [removeVisibility, { data, loading, error }] = useMutation(
    REMOVE_VISIBILITY,
    { refetchQueries: [{ query: query }] }
  )
  const [addVisibility, { data2, loading2, error2 }] =
    useMutation(ADD_VISIBILITY)
...
function Delete(props) {
  ...
  const handleClick = (e) => {
    if (
      confirm(
        'Haluatko varmasti poistaa postauksen ' +
          props.title +
          '?' +
